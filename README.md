...
## Ubuntu Setup (Native Linux)

Oh my, look at you! You're running Ubuntu (or a compatible distro), and you've got those big bear paws ready to type. Make sure you have `sudo` access, because we're going to be gettin' some fine little fishies.

### Run the Magic Script

1. Clone this repository to your workspace:
   ```bash
   git clone <this-repo> ~/workspace/bear-workspace
   cd ~/workspace/bear-workspace
   ```
...
4. Restart your terminal (or run `zsh`) to bask in the glory of your new shell

5. Open Neovim and run `:PlugInstall` to install all those beautiful plugins

---

## Windows Setup (WSL)

So you're on Windows, huh? That's okay, little bear. We don't judge. WSL lets you have the best of both worlds: Windows for... whatever Windows is good for, and Linux for actual work. Let's get you sorted.

### Install Ubuntu on Windows Subsystem for Linux

This is taken from: https://docs.microsoft.com/en-us/windows/wsl/install-win10
...
3. Run `./bear-with-me.sh`
4. Reload the terminal
5. Rock on

### Install Powerline Fonts (Windows)
...
2. Extract all from the zip
3. Open a Powershell as Administrator
4. Navigate to `fonts-master` directory: `cd "${HOME}\Downloads\fonts-master\fonts-master"`
5. Change the `Execution Policy`: `Set-ExecutionPolicy -Scope CurrentUser -ExecutionPolicy Bypass`
6. Run `.\install.ps1` script to install the fonts
7. Set your execution policy back to what it was before: `Set-ExecutionPolicy -Scope CurrentUser -ExecutionPolicy Undefined`
8. Change your Hyper font to `Fira Mono for Powerline`
...
### Vim Colors are Totally Uggo (Windows/WSL)
...
2. Scroll to the `Windows specifications` section
3. If `Version` is **less** than `1809`, then you need to update your version.

**Update Windows 10 Version**
1. Download the Windows provided [update installer](https://www.microsoft.com/en-us/software-download/windows10)
2. Run it, and it should install the latest version of Windows for you
...
2. Run `type AnsiColors256.ans` in Powershell
3. Change HyperJs to launch Powershell by default by editing the config
   - Hit `Ctrl` + `,` to open the HyperJS settings
...
1. Open vim
2. Type :checkhealth
3. You'll probably see this suggested help: `ERROR: Javascript entry not found, run ":call coc#util#install()" to fix it.`
4. Do the suggested help!
